...
const checkPermissions = async (message) => {
  try {
    ...
    await message.send("*ꜰᴀɪʟᴇᴅ ᴛᴏ ᴄʜᴇᴄᴋ `ᴘᴇʀᴍɪꜱꜱɪᴏɴ`*");
    return false;
  }
};
  ...
  try {
    ...
    for (const jid of jids) {
      ...
      if (status === 200 || status === "200") {
        successCount++;
        responseText += `*@${number} - ᴀᴅᴅᴇᴅ ꜱᴜᴄᴄᴇꜱꜱꜰᴜʟʟʏ*\n`;
      } else if (status === 403 || status === "403") {
        privacyBlocked++;
        responseText += `*@${number} - ᴘʀɪᴠᴀᴄʏ ꜱᴇᴛᴛɪɴɢ ʙʟᴏᴄᴋ*\n`;
      } else if (status === 409 || status === "409") {
        alreadyInGroup++;
        responseText += `*@${number} - ᴀʟʀᴇᴀᴅʏ ɪɴ ɢʀᴏᴜᴘ*\n`;
      } else {
        failedCount++;
    ...
    await message.react("❌");
    await message.send("*ᴇʀʀᴏʀ ᴅɪᴅɴ'ᴛ ᴀᴅᴅ `ᴍᴇᴍʙᴇʀ`*");
  }
});
...
Module({
  ...
  try {
    ...
    const jids = extractMultipleJids(message);
    if (jids.length === 0) {
      return message.send("*ᴛᴀɢ ᴏʀ ʀᴇᴘʟʏ ᴛᴏ ᴜꜱᴇʀ ᴛᴏ `ᴋɪᴄᴋ`*");
    }
    ...
    for (const jid of jids) {
      ...
      if (areJidsSame(message, jid, botJid)) {
        await message.send("*ᴄᴀɴɴᴏᴛ ᴋɪᴄᴋ `ᴍʏꜱᴇʟꜰ`*");
        continue;
      }

      // Check if trying to kick owner
      if (areJidsSame(message, jid, message.groupOwner)) {
        await message.send("*ᴄᴀɴɴᴏᴛ ᴋɪᴄᴋ ᴛʜᴇ ɢʀᴏᴜᴘ `ᴄʀᴇᴀᴛᴏʀ`*");
        continue;
      }
      ...
      if (isTargetAdmin && !message.isfromMe) {
        await message.send(`*ᴄᴀɴɴᴏᴛ ᴋɪᴄᴋ `ᴀᴅᴍɪɴ`* @${jid.split("@")[0]}_`, {
          mentions: [jid],
        });
        continue;
      }
    ...
    if (validJids.length === 0) {
      return message.send("*ɴᴏ ᴠᴀʟɪᴅ ᴜꜱᴇʀ ᴛᴏ `ᴋɪᴄᴋ`*");
    }
    ...
    await message.react("❌");
    await message.send("*ꜰᴀɪʟᴇᴅ ᴛᴏ ʀᴇᴍᴏᴠᴇ `ᴍᴇᴍʙᴇʀ`*");
  }
});
...
Module({
  ...
  try {
    ...
    const jids = extractMultipleJids(message);
    if (jids.length === 0) {
      return message.send("*ᴛᴀɢ ᴏʀ ʀᴇᴘʟʏ ᴛᴏ ᴜꜱᴇʀ ᴛᴏ `ᴘʀᴏᴍᴏᴛᴇ`*");
    }
    ...
    for (const jid of jids) {
      ...
      if (isAlreadyAdmin) {
        await message.send(`@${jid.split("@")[0]} *ᴀʟʀᴇᴀᴅʏ ᴀɴ ᴀᴅᴍɪɴ*`, {
          mentions: [jid],
        });
        continue;
      }
      ...
      if (!isInGroup) {
        await message.send(`❌ @${jid.split("@")[0]} *ɪꜱ ɴᴏᴛ ɪɴ ɢʀᴏᴜᴘ*`, {
          mentions: [jid],
        });
        continue;
      }
    ...
    if (validJids.length === 0) {
      return message.send("*ɴᴏ ᴠᴀʟɪᴅ ᴜꜱᴇʀ ᴛᴏ `ᴘʀᴏᴍᴏᴛᴇ`*");
    }
    ...
    await message.react("❌");
    await message.send("*ꜰᴀɪʟᴇᴅ ᴛᴏ ᴘʀᴏᴍᴏᴛᴇ `ᴍᴇᴍʙᴇʀ`*");
  }
});
...
Module({
  ...
  try {
    ...
    const jids = extractMultipleJids(message);
    if (jids.length === 0) {
      return message.send("*ᴛᴀɢ ᴏʀ ʀᴇᴘʟʏ ᴛᴏ ᴀɴ ᴀᴅᴍɪɴ ᴛᴏ `ᴅᴇᴍᴏᴛᴇ`*");
    }
    ...
    for (const jid of jids) {
      // Check if owner
      if (areJidsSame(message, jid, message.groupOwner)) {
        await message.send("*ᴄᴀɴɴᴏᴛ ᴅᴇᴍᴏᴛᴇ ᴛʜᴇ ɢʀᴏᴜᴘ `ᴄʀᴇᴀᴛᴏʀ`*");
        continue;
      }
      ...
      if (!isAdmin) {
        await message.send(`ℹ️ @${jid.split("@")[0]} *ɪs ɴᴏᴛ ᴀɴ ᴀᴅᴍɪɴ*`, {
          mentions: [jid],
        });
        continue;
      }
    ...
    if (validJids.length === 0) {
      return message.send("*ɴᴏ ᴠᴀʟɪᴅ ᴀᴅᴍɪɴꜱ ᴛᴏ `ᴅᴇᴍᴏᴛᴇ`*");
    }
    ...
    await message.react("❌");
    await message.send("*ꜰᴀɪʟᴇᴅ ᴛᴏ ᴅᴇᴍᴏᴛᴇ `ᴀᴅᴍɪɴ`*");
  }
});
...
Module({
  ...
  try {
    ...
    if (!message.announce) {
      return message.send("*ɢʀᴏᴜᴘ ɪꜱ ᴀʟʀᴇᴀᴅʏ `ᴏᴘᴇɴ`*");
    }
    ...
    await message.reply(
      "🔓 *Group Opened*\n\n*ᴀʟʟ ᴍᴇᴍʙᴇʀꜱ ᴄᴀɴ ɴᴏᴡ ꜱᴇɴᴅ `ᴍᴇꜱꜱᴀɢᴇꜱ`*"
    );
  } catch (error) {
...
Module({
  ...
  try {
    ...
    if (message.announce) {
      return message.send("*ɢʀᴏᴜᴘ ɪꜱ ᴀʟʀᴇᴀᴅʏ `ᴄʟᴏꜱᴇᴅ`*");
    }
    ...
    await message.reply(
      "🔒 *Group Closed*\n\n*ɴᴏᴡ ᴏɴʟʏ ᴀᴅᴍɪɴ ᴄᴀɴ ꜱᴇɴᴅ `ᴍᴇꜱꜱᴀɢᴇ`*"
    );
  } catch (error) {
...
Module({
  ...
  try {
    ...
    await message.reply(
      `*ʟɪɴᴋ ʀᴇᴠᴏᴋᴇᴅ*\n\n*ᴘʀᴇᴠɪᴏᴜꜱ ʟɪɴᴋ ɪꜱ ɴᴏᴡ ɪɴᴠᴀʟɪᴅ*\n\n*ɴᴇᴡ ʟɪɴᴋ:*\nhttps://chat.whatsapp.com/${newCode}`
    );
  } catch (error) {
    console.error("Revoke command error:", error);
    await message.send("*ꜰᴀɪʟᴇᴅ ᴛᴏ ʀᴇᴠᴏᴋᴇ ɪɴᴠɪᴛᴇ `ʟɪɴᴋ`*");
  }
});
...
Module({
  ...
  try {
    ...
    const text = match || "📢 *ᴀᴛᴛᴇɴᴛɪᴏɴ ᴇᴠᴇʀʏᴏɴᴇ*";
...
Module({
  ...
  try {
    ...
    const text = match || "*ᴀᴅᴍɪɴ ᴀᴛᴛᴇɴᴛɪᴏɴ `ɴᴇᴇᴅᴇᴅ`*";
    let tagText = `${text}\n\n`;
